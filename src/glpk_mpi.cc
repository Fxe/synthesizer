...
int main(int argc, char** argv)
{
  ...
  double speed = demo7(file, obj);

  MPI::COMM_WORLD.Barrier();

  cout << "demo7: " << speed << endl;
  cout << proc_name << " rank: " << rank << " world size: " << comm_size << endl;
  MPI::COMM_WORLD.Barrier();

  if (rank != 0)
  {
    //cout << rank << " sent data to 0" << endl;
    MPI::COMM_WORLD.Send(&speed, 1, MPI_DOUBLE, 0, 0);
  }
  else
  {
    ...
    for (int i = 1; i < comm_size; i++)
    {
      MPI_Status status;
      double recv;
      MPI::COMM_WORLD.Recv(&recv, 1, MPI_DOUBLE, i, 0);
      total_speed += recv;
    }
    cout << "total speed: " << total_speed << endl;
  }
